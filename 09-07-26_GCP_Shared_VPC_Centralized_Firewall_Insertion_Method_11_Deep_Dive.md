...
## 5.4 Representative PBR configuration
...
- expected Shared VPC network URI;
- expected source/destination ranges;
- expected priority;
- expected internal passthrough NLB next hop;
- intended traffic scope only.

**Failure indicators**

- wrong host-project network;
- overly broad source/destination range;
- firewall VM itself matches the same PBR;
- return traffic follows a bypass path.
