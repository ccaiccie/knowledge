...
- [1. The most important concept: inspection method and traffic-steering method are not the same thing](#1-the-most-important-concept-inspection-method-and-traffic-steering-method-are-not-the-same-thing)
- [2. Quick decision matrix](#2-quick-decision-matrix)
- [3. Method 1 — Azure Firewall in a customer-managed hub VNet](#3-method-1--azure-firewall-in-a-customer-managed-hub-vnet)
  - [3.1 Control plane](#31-control-plane)
  - [3.2 Data plane: spoke to Internet](#32-data-plane-spoke-to-internet)
  - [3.3 Data plane: spoke to spoke](#33-data-plane-spoke-to-spoke)
  - [3.4 Data plane: Azure to on-premises](#34-data-plane-azure-to-on-premises)
  - [3.5 Inbound publication through Azure Firewall DNAT](#35-inbound-publication-through-azure-firewall-dnat)
  - [3.6 Why this method is popular](#36-why-this-method-is-popular)
  - [3.7 Common failure modes](#37-common-failure-modes)
- [4. Method 2 — Third-party NGFW/NVA in a customer-managed hub VNet](#4-method-2--third-party-ngfwnva-in-a-customer-managed-hub-vnet)
  - [4.1 Single NVA — simplest but not production-resilient](#41-single-nva--simplest-but-not-production-resilient)
  - [4.2 Redundant NVA pair behind Standard Load Balancer HA Ports](#42-redundant-nva-pair-behind-standard-load-balancer-ha-ports)
- [5. Method 3 — Azure Route Server + third-party NVA for dynamic service insertion](#5-method-3--azure-route-server--third-party-nva-for-dynamic-service-insertion)
  - [5.1 Why use it](#51-why-use-it)
  - [5.2 Critical caveat](#52-critical-caveat)
  - [5.3 High availability](#53-high-availability)
- [6. Method 4 — Azure Virtual WAN secured hub with Azure Firewall](#6-method-4--azure-virtual-wan-secured-hub-with-azure-firewall)
  - [6.1 Routing Intent](#61-routing-intent)
  - [6.2 Why Routing Intent is superior to ad hoc static routing for global vWAN inspection](#62-why-routing-intent-is-superior-to-ad-hoc-static-routing-for-global-vwan-inspection)
  - [6.3 Static routing without Routing Intent](#63-static-routing-without-routing-intent)
- [7. Method 5 — Integrated third-party NGFW directly inside the Virtual WAN hub](#7-method-5--integrated-third-party-ngfw-directly-inside-the-virtual-wan-hub)
  - [7.1 Current documented security NVA categories](#71-current-documented-security-nva-categories)
  - [7.2 Coexistence with Azure Firewall](#72-coexistence-with-azure-firewall)
  - [7.3 Licensing](#73-licensing)
- [8. Method 6 — Virtual WAN security SaaS provider](#8-method-6--virtual-wan-security-saas-provider)
- [9. Method 7 — Gateway Load Balancer for transparent NVA insertion](#9-method-7--gateway-load-balancer-for-transparent-nva-insertion)
  - [9.1 Why it is different from UDR-based insertion](#91-why-it-is-different-from-udr-based-insertion)
  - [9.2 Traffic model](#92-traffic-model)
  - [9.3 Good use cases](#93-good-use-cases)
  - [9.4 Poor use cases](#94-poor-use-cases)
- [10. Method 8 — Forced tunneling: inspect Internet traffic on-premises](#10-method-8--forced-tunneling-inspect-internet-traffic-on-premises)
  - [10.1 Reasons to use forced tunneling](#101-reasons-to-use-forced-tunneling)
  - [10.2 Tradeoffs](#102-tradeoffs)
- [11. Method 9 — Layer-7 web firewall inspection with Azure Front Door WAF and Application Gateway WAF](#11-method-9--layer-7-web-firewall-inspection-with-azure-front-door-waf-and-application-gateway-waf)
  - [11.1 Azure Front Door WAF](#111-azure-front-door-waf)
  - [11.2 Application Gateway WAF](#112-application-gateway-waf)
  - [11.3 Ordering with Azure Firewall](#113-ordering-with-azure-firewall)
- [12. Method 10 — Inspect traffic to Azure Private Endpoints](#12-method-10--inspect-traffic-to-azure-private-endpoints)
  - [12.1 Application rules versus network rules](#121-application-rules-versus-network-rules)
  - [12.2 DNS is part of the security path](#122-dns-is-part-of-the-security-path)
- [13. Advanced Virtual WAN pattern — Azure Firewall plus a spoke NVA](#13-advanced-virtual-wan-pattern--azure-firewall-plus-a-spoke-nva)
- [14. Azure Firewall feature depth: what “inspection” actually means](#14-azure-firewall-feature-depth-what-inspection-actually-means)
  - [14.1 Network rules](#141-network-rules)
  - [14.2 Application rules](#142-application-rules)
  - [14.3 DNAT rules](#143-dnat-rules)
  - [14.4 Threat intelligence](#144-threat-intelligence)
  - [14.5 Premium TLS inspection](#145-premium-tls-inspection)
  - [14.6 Premium IDPS](#146-premium-idps)
  - [14.7 URL filtering and web categories](#147-url-filtering-and-web-categories)
- [15. Control plane versus data plane for every architecture](#15-control-plane-versus-data-plane-for-every-architecture)
- [16. Packet/session-flow principles that apply to every stateful firewall](#16-packetsession-flow-principles-that-apply-to-every-stateful-firewall)
  - [16.1 Symmetry](#161-symmetry)
  - [16.2 Longest-prefix routing wins](#162-longest-prefix-routing-wins)
  - [16.3 NAT changes what downstream systems see](#163-nat-changes-what-downstream-systems-see)
  - [16.4 DNS can change the route](#164-dns-can-change-the-route)
- [17. High availability and convergence](#17-high-availability-and-convergence)
- [18. Configuration patterns](#18-configuration-patterns)
  - [18.1 UDR to a firewall/NVA](#181-udr-to-a-firewallnva)
  - [18.2 Azure Route Server peer](#182-azure-route-server-peer)
  - [18.3 NAT Gateway attached to AzureFirewallSubnet](#183-nat-gateway-attached-to-azurefirewallsubnet)
- [19. Verification workflow](#19-verification-workflow)
- [20. Troubleshooting by symptom](#20-troubleshooting-by-symptom)
- [21. Architecture-selection recommendations](#21-architecture-selection-recommendations)
- [22. Common design mistakes](#22-common-design-mistakes)
- [23. Final design checklist](#23-final-design-checklist)
...
**Click any method name to jump directly to the section that explains it. Use the dedicated deep-dive link for the standalone 09-05 study guide when one exists.**

| Method | Dedicated 09-05 deep dive | Typical inspector | Best for | East-west | Internet egress | Internet ingress | Hybrid/branch | Main steering mechanism |
|---|---|---|---|---:|---:|---:|---:|---|
| [Hub VNet + Azure Firewall](#3-method-1--azure-firewall-in-a-customer-managed-hub-vnet) | [Method 1 deep dive](09-05-26-18-55_Azure_Firewall_Customer_Managed_Hub_VNet_Method_1_Deep_Dive.md) | Azure Firewall | Classic enterprise hub-spoke | Yes | Yes | Yes, DNAT | Yes | UDR + VNet peering/gateway transit |
| [Hub VNet + third-party NVA](#4-method-2--third-party-ngfwnva-in-a-customer-managed-hub-vnet) | [Method 2 deep dive](09-05-26-19-45_Third_Party_NGFW_NVA_Customer_Managed_Hub_VNet_Method_2_Deep_Dive.md) | Palo Alto/Fortinet/Check Point/Cisco/etc. | Feature-rich NGFW requirements | Yes | Yes | Yes | Yes | UDR, ILB, BGP, Route Server |
| [Azure Route Server + NVA](#5-method-3--azure-route-server--third-party-nva-for-dynamic-service-insertion) | [Method 3 deep dive](09-05-26-13-55_Azure_Route_Server_Third_Party_NVA_Dynamic_Service_Insertion_Study_Guide.md) | Third-party NVA | Dynamic BGP-based service insertion | Yes | Yes | Possible | Yes | BGP route exchange |
| [Virtual WAN secured hub](#6-method-4--azure-virtual-wan-secured-hub-with-azure-firewall) | [Method 4 deep dive](09-05-26-15-56_Azure_Virtual_WAN_Secured_Hub_Method_4_Study_Guide.md) · [Multi-region companion](09-05-26-16-05_Azure_Virtual_WAN_Multi_Region_Hubs_Deep_Dive.md) | Azure Firewall | Managed global transit + centralized policy | Yes | Yes | Possible | Yes | Routing Intent / hub policy |
| [Virtual WAN integrated NGFW](#7-method-5--integrated-third-party-ngfw-directly-inside-the-virtual-wan-hub) | [Method 5 deep dive](09-05-26-20-00_Azure_Virtual_WAN_Integrated_Third_Party_NGFW_Direct_Hub_Deep_Dive.md) | Qualified integrated NVA | Managed vWAN + third-party NGFW | Yes | Yes | Vendor-dependent | Yes | Routing Intent |
| [Virtual WAN + security SaaS](#8-method-6--virtual-wan-security-saas-provider) | [Method 6 deep dive](09-05-26-16-44_Azure_Virtual_WAN_Security_SaaS_Provider_Method_6_Deep_Dive.md) | Security partner service | Cloud-delivered internet/private inspection | Policy-dependent | Yes | Usually not app publication | Yes | Routing Intent |
| [Standard LB HA Ports + NVA pair](#42-redundant-nva-pair-behind-standard-load-balancer-ha-ports) | Covered in [Method 2 deep dive](09-05-26-19-45_Third_Party_NGFW_NVA_Customer_Managed_Hub_VNet_Method_2_Deep_Dive.md) | Third-party NVA | Active-active/active-passive firewall HA | Yes | Yes | Yes | Yes | UDR/BGP to ILB VIP |
| [Gateway Load Balancer](#9-method-7--gateway-load-balancer-for-transparent-nva-insertion) | [Method 7 deep dive](09-05-26-17-03_Gateway_Load_Balancer_Transparent_NVA_Insertion_Study_Guide.md) | Third-party transparent NVA | Transparent insertion on public endpoints | Limited to chained path | Yes | Yes | Not general hub transit | Service chaining / VXLAN |
| [Forced tunneling to on-prem](#10-method-8--forced-tunneling-inspect-internet-traffic-on-premises) | [Method 8 deep dive](09-05-26-20-53_Azure_Forced_Tunneling_On_Premises_Internet_Inspection_Deep_Dive.md) | Azure Firewall and/or on-prem NGFW | Centralized corporate internet egress | Yes locally | Yes, on-prem | Generally no Azure Firewall DNAT in FT design | Yes | Default route to gateway/on-prem |
| [Front Door WAF / App Gateway WAF](#11-method-9--layer-7-web-firewall-inspection-with-azure-front-door-waf-and-application-gateway-waf) | No standalone 09-05 deep dive found | WAF | HTTP/HTTPS threat inspection | No generic L3/L4 east-west | No generic egress | Yes | No generic transit | Reverse proxy |
| [Private Endpoint inspection](#12-method-10--inspect-traffic-to-azure-private-endpoints) | No standalone 09-05 deep dive found | Azure Firewall / NVA | Private Link governance | Yes for PE flows | N/A | N/A | Yes | UDR/routing intent + PE network policies |
