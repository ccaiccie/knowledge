...
replacements = {
    'images/09-05-26-20-00_integrated_ngfw_architecture.svg': 'images/09-05-26-20-00_integrated_ngfw_architecture_v2.svg',
    'images/09-05-26-20-00_integrated_ngfw_architecture.drawio': 'images/09-05-26-20-00_integrated_ngfw_architecture_v2.drawio',
    'images/09-05-26-20-00_routing_intent_control_plane.svg': 'images/09-05-26-20-00_routing_intent_control_plane_v2.svg',
    'images/09-05-26-20-00_routing_intent_control_plane.drawio': 'images/09-05-26-20-00_routing_intent_control_plane_v2.drawio',
}
for old, new in replacements.items():
    text = text.replace(old, new)

old_private = '''![Private packet flow](images/09-05-26-20-00_private_packet_flow.svg)
...
'''
new_private = '''### Forward-path diagram
...
if old_private not in text:
    raise SystemExit('Expected old private-flow block not found')
text = text.replace(old_private, new_private, 1)

internet_anchor = 'For VM-A `10.10.1.4` to `8.8.8.8:443`:\n\n'
internet_forward = '''For VM-A `10.10.1.4` to `8.8.8.8:443`:
...
if internet_anchor not in text:
    raise SystemExit('Internet egress anchor not found')
text = text.replace(internet_anchor, internet_forward, 1)

return_anchor = '6. Return packets land on the security service, reverse NAT/state is applied, and the vHub returns traffic to the spoke.\n\n**Common failure:**'
internet_return = '''6. Return packets land on the security service, reverse NAT/state is applied, and the vHub returns traffic to the spoke.
...
if return_anchor not in text:
    raise SystemExit('Internet return anchor not found')
text = text.replace(return_anchor, internet_return, 1)

old_dnat = '''![DNAT packet flow](images/09-05-26-20-00_dnat_packet_flow.svg)
...
'''
new_dnat = '''### Inbound / forward-path diagram
...
'''
if old_dnat not in text:
    raise SystemExit('Expected old DNAT-flow block not found')
text = text.replace(old_dnat, new_dnat, 1)

GUIDE.write_text(text, encoding='utf-8')

r = README.read_text(encoding='utf-8')
title = '### [Integrated Third-Party NGFW Directly Inside an Azure Virtual WAN Hub — Deep Dive]'
pos = r.find(title)
if pos < 0:
    raise SystemExit('README guide entry not found')
end = r.find('\n\n### ', pos + len(title))
if end < 0:
    end = len(r)
entry = r[pos:end]
old = 'with four matching SVG and editable draw.io diagrams.'
new = 'with redesigned color-coded architecture/control-plane diagrams and independent forward/return SVG + editable draw.io packet-flow pairs for east-west, Internet egress, and Internet-inbound DNAT.'
if old not in entry:
    raise SystemExit('README old diagram summary not found')
entry = entry.replace(old, new, 1)
r = r[:pos] + entry + r[end:]
README.write_text(r, encoding='utf-8')

# Validate all diagram links and all redesigned pairs.
import re
refs = sorted(set(re.findall(r'\(images/([^)]*\.(?:svg|drawio))\)', text)))
missing = [x for x in refs if not (Path('images') / x).exists()]
if missing:
    raise SystemExit('Missing diagram assets: ' + ', '.join(missing))
for stem in [
    '09-05-26-20-00_integrated_ngfw_architecture_v2',
    '09-05-26-20-00_routing_intent_control_plane_v2',
    '09-05-26-20-00_eastwest_forward',
    '09-05-26-20-00_eastwest_return',
    '09-05-26-20-00_internet_egress_forward',
    '09-05-26-20-00_internet_egress_return',
    '09-05-26-20-00_dnat_inbound_forward',
    '09-05-26-20-00_dnat_inbound_return',
]:
    if not (Path('images') / f'{stem}.svg').exists() or not (Path('images') / f'{stem}.drawio').exists():
        raise SystemExit('Missing pair for ' + stem)
print(f'Validated {len(refs)} diagram references and all redesigned pairs.')
