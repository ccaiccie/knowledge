...
### [GCP Cloud NGFW Distributed Policy — Architecture, gcloud Deployment, Packet Flow, Enterprise L7 Inspection, Verification, and Troubleshooting](09-06-26-19-21_GCP_Cloud_NGFW_Distributed_Policy_Deep_Dive.md)
Deep dive into Google Cloud NGFW distributed enforcement using hierarchical and global/regional network firewall policies, secure-tag microsegmentation, documented rule-evaluation order, policy-first `BEFORE_CLASSIC_FIREWALL`, east-west and north-south packet flow, Cloud NAT separation, Cloud NGFW Enterprise zonal firewall endpoints and Packet Intercept, threat-prevention security profiles, optional TLS inspection, `gcloud` deployment/verification, HA/MTU/capacity limits, and symptom-based troubleshooting, with three matching SVG/editable draw.io diagrams.

### [Palo Alto Networks Firewalling in Google Cloud — Cloud NGFW Enterprise Integration vs VM-Series Service Insertion](09-07-26-07-03_Palo_Alto_Networks_Firewalling_in_GCP_Deep_Dive.md)
Separates Google Cloud NGFW Enterprise from customer-deployed Palo Alto VM-Series, then deeply covers VM-Series service insertion using Google Cloud Network Security Integration (NSI) and the traditional internal passthrough NLB + PBR/custom-route model. Includes GENEVE interception/reinjection, producer/consumer objects, `gcloud` deployment, PAN-OS GENEVE enablement, east-west and Internet packet flows, NAT boundaries, HA/autoscaling, verification, common mistakes, troubleshooting, and three matching SVG/editable draw.io diagrams.
