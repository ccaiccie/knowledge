...
- [1. What Method 1 actually is](#1-what-method-1-actually-is)
- [2. Reference topology used throughout this guide](#2-reference-topology-used-throughout-this-guide)
- [3. The fundamental routing model](#3-the-fundamental-routing-model)
  - [3.1 Azure Firewall does not magically attract traffic](#31-azure-firewall-does-not-magically-attract-traffic)
  - [3.2 Longest-prefix match comes first](#32-longest-prefix-match-comes-first)
  - [3.3 UDR, BGP, and system-route interaction](#33-udr-bgp-and-system-route-interaction)
- [4. Route-table design by traffic class](#4-route-table-design-by-traffic-class)
  - [4.1 Example Spoke A route table](#41-example-spoke-a-route-table)
  - [4.2 Example Spoke B route table](#42-example-spoke-b-route-table)
  - [4.3 Why both directions matter](#43-why-both-directions-matter)
- [5. VNet peering settings in detail](#5-vnet-peering-settings-in-detail)
- [6. Packet flow: Spoke → Internet](#6-packet-flow-spoke--internet)
- [7. Azure Firewall + NAT Gateway](#7-azure-firewall--nat-gateway)
- [8. Packet flow: Spoke A → Spoke B](#8-packet-flow-spoke-a--spoke-b)
- [9. Packet flow: Spoke → on-premises](#9-packet-flow-spoke--on-premises)
  - [9.1 Why GatewaySubnet routing matters](#91-why-gatewaysubnet-routing-matters)
  - [9.2 Gateway route propagation](#92-gateway-route-propagation)
- [10. Packet flow: Internet ingress using DNAT](#10-packet-flow-internet-ingress-using-dnat)
- [11. Azure Firewall rule-processing behavior](#11-azure-firewall-rule-processing-behavior)
  - [11.1 Network rules](#111-network-rules)
  - [11.2 Application rules](#112-application-rules)
  - [11.3 Default deny](#113-default-deny)
- [12. Standard vs Premium feature considerations](#12-standard-vs-premium-feature-considerations)
  - [12.1 TLS inspection](#121-tls-inspection)
  - [12.2 IDPS](#122-idps)
- [13. Forced tunneling](#13-forced-tunneling)
- [14. DNS and application-rule dependencies](#14-dns-and-application-rule-dependencies)
- [15. Private Endpoint considerations](#15-private-endpoint-considerations)
- [16. High availability and Azure Firewall scaling](#16-high-availability-and-azure-firewall-scaling)
- [17. Configuration example — Azure CLI for route steering](#17-configuration-example--azure-cli-for-route-steering)
- [18. Example routing matrix before implementation](#18-example-routing-matrix-before-implementation)
- [19. Verification workflow](#19-verification-workflow)
  - [19.1 Verify route-table objects](#191-verify-route-table-objects)
  - [19.2 Verify effective routes on the NIC](#192-verify-effective-routes-on-the-nic)
  - [19.3 Verify Network Watcher exact next hop](#193-verify-network-watcher-exact-next-hop)
  - [19.4 Verification logic in one table](#194-verification-logic-in-one-table)
  - [19.5 Verify firewall logs](#195-verify-firewall-logs)
- [20. Troubleshooting by symptom](#20-troubleshooting-by-symptom)
  - [Symptom A — Spoke can reach Internet but firewall shows no log](#symptom-a--spoke-can-reach-internet-but-firewall-shows-no-log)
  - [Symptom B — Spoke A → Spoke B bypasses firewall](#symptom-b--spoke-a--spoke-b-bypasses-firewall)
  - [Symptom C — TCP SYN reaches destination, return traffic fails](#symptom-c--tcp-syn-reaches-destination-return-traffic-fails)
  - [Symptom D — Azure → on-premises works, on-premises → Azure fails](#symptom-d--azure--on-premises-works-on-premises--azure-fails)
  - [Symptom E — DNAT listener is reachable but application hangs](#symptom-e--dnat-listener-is-reachable-but-application-hangs)
  - [Symptom F — FQDN application rule appears ignored](#symptom-f--fqdn-application-rule-appears-ignored)
  - [Symptom G — Only some Internet sessions fail under load](#symptom-g--only-some-internet-sessions-fail-under-load)
  - [Symptom H — TLS inspection breaks one SaaS application](#symptom-h--tls-inspection-breaks-one-saas-application)
- [21. Common mistakes](#21-common-mistakes)
- [22. When Method 1 is the right architecture](#22-when-method-1-is-the-right-architecture)
- [23. When another method can be better](#23-when-another-method-can-be-better)
- [24. Design checklist](#24-design-checklist)
